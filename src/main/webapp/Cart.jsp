...
<body>
    ...
    <table>
    <sql:query var="reqClients" sql="SELECT * FROM clients where id = 2" />
    <c:set var="theProducts" value="${reqClients.rows[0].cart}"/>
          <c:set var = "string2" value = "${fn:split(theProducts, ',')}" />
   <table>
   
    <c:forEach var = "i" items="${string2}">
  <tr>
  		
	    <sql:query var="row" sql="SELECT * FROM products where id = ${i}" />
                <td>
                    <div class="productContainer">
                        <a href="">
                            <img src="${row.rows[0].picture}" class="product" alt="">
                            <p>${row.rows[0].name}</p>
                            <p>${row.rows[0].price}</p>
                        </a>
                    </div>
                </td>
</tr>
